...
namespace ArcsNeuron {
	...
	struct AutoDiffData {
		...
		std::function<void*(const std::any)> ConvTo_u1ptr;	//!< std::any → ArcsNeu<T>*アドレス変換関数オブジェクト
		std::function<void*(const std::any)> ConvTo_u2ptr;	//!< std::any → ArcsNeu<T>*アドレス変換関数オブジェクト
		std::function<void*(const std::any)> ConvTo_yptr;	//!< std::any → ArcsNeu<T>*アドレス変換関数オブジェクト
	...
	public:
			...
			: Stack({}), StackCounter(0), TempObjStack({}), TempObjStackCounter(0)
		{
			...
			for(size_t i = 0; i < MAX_OPERATION; ++i){
				...
				Stack.at(i).ConvTo_u1ptr = nullptr;
				Stack.at(i).ConvTo_u2ptr = nullptr;
				Stack.at(i).ConvTo_yptr  = nullptr;
		...
		void DispStack(void) const{
			...
			for(size_t i = 0; i < StackCounter; ++i){
				printf("%5zu: %p %s %p -> [%p] %p\n", i,
					 Stack.at(i).ConvTo_u1ptr(Stack.at(i).u1), GetOperatorName(i).c_str(), Stack.at(i).ConvTo_u2ptr(Stack.at(i).u2),
					 &(Stack.at(i).y), Stack.at(i).ConvTo_yptr(Stack.at(i).y)
				);
			}
		}
...
class ArcsNeu {
	public:
		...
		constexpr ArcsNeu(void) noexcept
			: value(0), grad(0), AutoDiffStack(nullptr), YaddrInStack(nullptr), YaddrInStack2(nullptr)
		{
			// 初期化以外の処理は無し
		}

		//! @brief コンストラクタ
		constexpr ArcsNeu(ArcsNeuStack* AutoDiff) noexcept
			: value(0), grad(0), AutoDiffStack(AutoDiff), YaddrInStack(nullptr), YaddrInStack2(nullptr)
		{
			// 初期化以外の処理は無し
		}
		...
		constexpr ArcsNeu(const ArcsNeu<T>& right) noexcept
			: value(right.value), grad(right.grad), AutoDiffStack(right.AutoDiffStack), YaddrInStack(right.YaddrInStack), YaddrInStack2(right.YaddrInStack2)
		...
		constexpr auto operator+(ArcsNeu<R>& right) & {
			...
			// 出力の計算
			auto outval = value + right.value;				// 出力のデータ型を確定させる
			ArcsNeu<decltype(outval)> ret(AutoDiffStack);	// 出力エッジ変数オブジェクト
			ret.value = outval;								// 出力値を格納しておく
			
			// 自動微分スタック
			ArcsNeuron::AutoDiffData ADret = {
				.OperatorType    = ArcsNeuron::AutoDiffOperator::ANE_ADD,	// 加算を指定
				.ForwardOperator = forward_add,			// 順方向演算の関数オブジェクトを指定
				.BackOperator    = backward_add<R, decltype(outval)>,		// 逆方向演算の関数オブジェクトを指定
				.ConvTo_u1ptr = (*this).conv_to_ptr,	// std::any → ArcsNeu<T_u1>*アドレス変換関数オブジェクト
				.ConvTo_u2ptr = right.conv_to_ptr,		// std::any → ArcsNeu<T_u2>*アドレス変換関数オブジェクト
				.ConvTo_yptr  = ret.conv_to_ptr,		// std::any → ArcsNeu<T_y>* アドレス変換関数オブジェクト
			...
			// 一時オブジェクトへの対応
			ret.YaddrInStack2 = AutoDiffStack->GetYaddress();
		...
		constexpr void SetAutoDiffStack(ArcsNeuStack* const ADStack){
		...
		static constexpr void forward_add(const std::any u1, const std::any u2, std::any y){
			(*std::any_cast<ArcsNeu<T>*>(y)).value =
				(*std::any_cast<ArcsNeu<T>*>(u1)).value + (*std::any_cast<ArcsNeu<T>*>(u2)).value;
		}
	...
	private:
		ArcsNeuStack* AutoDiffStack;	//! 自動微分スタックへの生ポインタ
		ArcsNeu<T>** YaddrInStack;		//! 自動微分スタック内の出力変数への生ポインタのアドレス (廃止)
		std::any* YaddrInStack2;		//! 自動微分スタック内の出力変数への生ポインタのアドレス
};
