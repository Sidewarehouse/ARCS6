...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	//static constexpr Matrix<1,ZFA01::N_AX> Kpt = {0.7, 0.6, 0.5};	// [-] ねじれトルク制御 比例ゲイン
	static constexpr Matrix<1,ZFA01::N_AX> Kpt = {0, 0, 0.5};	// [-] ねじれトルク制御 比例ゲイン
	...
	static Matrix<1,ZFA01::N_AX> alref;	// [rad/s^2] 負荷側加速度指令
	static Matrix<1,ZFA01::N_AX> amref;	// [rad/s^2] モータ側加速度指令
	...
	if(CmdFlag == CTRL_LOOP){
		...
		alref = (tsref - taus) & Kpt;				// [rad/s^2] 負荷側加速度指令演算
		amref = alref & Rg;							// [rad/s^2] モータ側加速度指令演算
