...
class InterfaceFunctions {
	public:
		...
		USV_PCIE7<EquipParams::ACTUATOR_NUM> Uniservo;				// ユニサーボIFボードクラス
		...
		void GetPositionAndVelocity(
			...
			std::array<double, EquipParams::ACTUATOR_NUM> EncPosition, PositionRes, VelocityRes;
			...
			Position.LoadArray(PositionRes);
			Velocity.LoadArray(VelocityRes);
		}
		...
		void GetTorque(ArcsMat<EquipParams::ACTUATOR_NUM, 1>& Torque){
			...
			std::array<double, EquipParams::ACTUATOR_NUM> TorqueRes;
			Uniservo.GetTorque(TorqueRes);
			Torque.LoadArray(TorqueRes);
		}
