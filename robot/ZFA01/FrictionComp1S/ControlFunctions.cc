...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	[[maybe_unused]] static constexpr double Ktn = ZFA01::Kt[1];// [Nm/A] トルク定数
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// 関節空間電流制御
		iqref[1] = AmpIncSquareWave(t, 0.25, 0.05, 30) + ZFA01::Fclmb[1]*tanh(2.0*wm[1])/ZFA01::Kt[1];	// [A] 1S電流指令
		iqref[2] = 0;	// [A] 2L電流指令
		iqref[3] = 0;	// [A] 3U電流指令
		...
		Memory.SetData(Tact, t, iqref[1], wm[1]);						// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
