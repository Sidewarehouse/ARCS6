...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	constexpr double fmin = 3;		// [Hz] FRA開始周波数
	constexpr double fmax = 100; 	// [Hz] FRA終了周波数
	constexpr double fstp =  1;		// [Hz] FRA周波数ステップ
	constexpr double Nint = 10;		// [-]  FRA積分周期 (1周波数につき何回sin波を入力するか)
	constexpr double Iamp = 0.2;	// [A]  FRA振幅
	constexpr double Ibias = -0.43;	// [A]  FRAバイアス
	constexpr double tsta = 0;		// [s]  FRA開始時刻
	...
	static double ifra = 0;				// [A]  FRA励起電流
	static double ffra = 1;				// [Hz] FRA周波数
	
	static ZFA01 Robot;					// ZFA01ロボット制御クラス
	static FRAgenerator FRA(fmin, fmax, fstp, Nint, Iamp, Ibias, tsta);	// FRA信号生成器
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// ここに制御アルゴリズムを記述する
		std::tie(ffra, ifra) = FRA.GetSignal(t);
		iqref[1] = 0;		// [A] 1S電流指令
		iqref[2] = 0;		// [A] 2L電流指令
		iqref[3] = ifra;	// [A] 3U電流指令
		...
		Screen.SetVarIndicator(ffra, ifra, wm[1], taus[1], 0, 0, 0, 0, 0, 0);	// 任意変数インジケータ(変数0, ..., 変数9)
		...
		Memory.SetData(Tact, t, ffra, ifra, wm[3], taus[3]);			// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
