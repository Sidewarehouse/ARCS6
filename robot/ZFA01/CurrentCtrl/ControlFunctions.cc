...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// ここに制御アルゴリズムを記述する
		iqref[1] = -StairsWave(t, 0.5, 0.1, 0.5, 15) - 0.5;//  1.4*sin(2.0*M_PI*0.25*t);	// [A] 1S電流指令
		iqref[2] = 0;//-StairsWave(t, 0.5, 0.1, 0.5, 10);// 1.4*sin(2.0*M_PI*0.25*t);	// [A] 2L電流指令
		iqref[3] = 0;//StairsWave(t, 0.5, 0.1, 0.5, 20);// 0.3*sin(2.0*M_PI*0.25*t);	// [A] 3U電流指令
