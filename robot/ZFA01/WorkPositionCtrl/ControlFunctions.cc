...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	static Matrix<1,ZFA01::N_AX> amref;	// [rad/s] モータ側加速度指令
	...
	if(CmdFlag == CTRL_LOOP){
		...
		if(0 <= t && t < 2){
			...
			// 2～7秒は指定位置へ移動
			pGptp[1] = ZFA01::pGhome[1] + 300e-3;	// [m] X軸移動先
			pGptp[2] = ZFA01::pGhome[2];			// [m] Y軸移動先
			pGptp[3] = ZFA01::pGhome[3];			// [m] Z軸移動先
		}else{
			pGptp = ZFA01::pGhome;		// [m] 7秒以降はホームポジションに戻る
		}
		pGref = PosRefShaper.GetShapedSignal(pGptp);	// [m] 位置指令(S字成形後)
		
		// 作業空間位置制御（位置P＋速度P＋外乱オブザーバ，3軸分）
		...
		Memory.SetData(Tact, t, pGref[1], pG[1], pGref[2], pG[2], pGref[3], pG[3], 0, 0, 0);	// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
