...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	constexpr double fmin = 1;		// [Hz] FRA開始周波数
	constexpr double fmax = 100; 	// [Hz] FRA終了周波数
	constexpr double fstp = 0.5;	// [Hz] FRA周波数ステップ
	constexpr double Nint = 10;		// [-]  FRA積分周期 (1周波数につき何回sin波を入力するか)
	constexpr double Iamp = 1.5;	// [A]  FRA振幅
	constexpr double Ibias = 0;		// [A]  FRAバイアス
	...
	if(CmdFlag == CTRL_LOOP){
		...
		Memory.SetData(Tact, t, ffra, ifra, wm[1], taus[1]);			// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
