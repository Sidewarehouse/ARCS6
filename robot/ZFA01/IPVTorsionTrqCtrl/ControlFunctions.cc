...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	static Matrix<1,ZFA01::N_AX> tsref;	// [Nm] ねじれトルク指令
	
	static ZFA01 Robot;					// ZFA01ロボット制御クラス
	
	static constexpr double Km = -0.0602;
	static constexpr double Kp = -0.4205;
	static constexpr double Ki = 15.4319;
	static constexpr double gi =  1.1798;
	static constexpr Matrix<1,1> Ncon = {1};
	static constexpr Matrix<1,2> Dcon = {1, gi};
	static TransferFunction<0,1> Icon(Ncon, Dcon, Ts);	// 不完全積分器
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// 関節空間ねじれトルク制御（ねじれトルクP＋加速度制御，3軸分）
		iqref[1] = 0;
		iqref[2] = 0;
		iqref[3] = Ki*Icon.GetResponse(tsref[3] - taus[3]) + Kp*taus[3] + Km*wm[3];	// [A]  電流指令 ←うごかねぇなコレ
		...
		Memory.SetData(Tact, t, tsref[1], taus[1], tsref[2], taus[2], tsref[3], taus[3], thm[1], thm[2], thm[3]);	// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
