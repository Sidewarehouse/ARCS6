...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	static constexpr Matrix<1,ZFA01::N_AX> Kpv  = {100, 100, 100};	// [rad/s] 速度制御比例ゲイン(=速度制御帯域)
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// 速度指令生成
		wmref[1] = SquareWave(0.25, 0, t, 1)*Rg[1]*M_PI/180.0*3.0;	// [rad/s] 速度指令 1S軸
		wmref[2] = SquareWave(0.25, 0, t, 1)*Rg[2]*M_PI/180.0*3.0;	// [rad/s] 速度指令 2L軸
		wmref[3] = 0;	// [rad/s] 速度指令 3U軸
		
		// 関節空間速度制御（速度P＋外乱オブザーバ，3軸分）
		...
		Memory.SetData(Tact, t, wmref[2], wm[2], 0, 0, 0, 0, 0, 0, 0);	// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
