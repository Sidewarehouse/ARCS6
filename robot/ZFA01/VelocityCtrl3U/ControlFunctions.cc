...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	static constexpr double Ktn = ZFA01::Kt[3];	// [Nm/A] トルク定数
	static constexpr double gdis = 300;	// [rad/s] 外乱オブザーバの推定帯域
	static constexpr double Kpv = 100;	// [rad/s] 速度制御比例ゲイン(=速度制御帯域)
	...
	static double wmref = 0;			// [rad/s] 速度指令
	static double amref = 0;			// [rad/s] 加速度指令
	static double tdis = 0;				// [Nm] 推定外乱
	static double Jan = ZFA01::Ja[3];	// [kgm^2] 全慣性
	
	static ZFA01 Robot;					// ZFA01ロボット制御クラス
	static DisturbanceObsrv<DObType::FULL_0TH> DOB(Ktn, Jan, gdis, Ts);	// 外乱オブザーバ
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// 速度指令生成
		wmref = -(SquareWave(0.25, 0, t, 1) + 1)*Rg[3]*M_PI/180.0*1.0;	// [rad/s] 速度指令
		
		// 関節空間速度制御（速度P＋外乱オブザーバ）
		amref = (wmref - wm[3])*Kpv;				// [rad/s^2] 加速度指令演算
		tdis = DOB.GetDistTorque(iqref[3], wm[3]);	// [Nm] 推定外乱演算
		iqref[1] = 0;	// [A] 1S電流指令
		iqref[2] = 0;	// [A] 2L電流指令
		iqref[3] = amref*Jan/Ktn + tdis/Ktn;	// [A] 3U電流指令
		...
		Memory.SetData(Tact, t, wmref, wm[3], 0, 0, 0, 0, 0, 0, 0);		// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
