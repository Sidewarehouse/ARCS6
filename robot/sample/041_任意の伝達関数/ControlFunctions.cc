...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	constexpr Matrix<1,3> N = {5,6,7};	// 伝達関数1の分子の係数ベクトル (5*s^2 + 6*s + 7)
	constexpr Matrix<1,4> D = {2,3,4,5};// 伝達関数1の分母の係数ベクトル (2*s^3 + 3*s^2 + 4*s + 5)
	constexpr Matrix<1,4> P = {4,5,6,7};// 伝達関数2の分子の係数ベクトル (4*s^3 + 5*s^2 + 6*s + 7)
	constexpr Matrix<1,4> Q = {2,3,4,5};// 伝達関数2の分母の係数ベクトル (2*s^3 + 3*s^2 + 4*s + 5)
	constexpr double w = 30;					// [rad/s] 2次遅れ系の周波数
	constexpr double z = 0.2;					// [-] 2次遅れ系の制動係数
	constexpr Matrix<1,1> B = {w*w};			// 2次遅れ系の分子係数ベクトル (ω^2)
	constexpr Matrix<1,3> A = {1, 2.0*z*w, w*w};// 2次遅れ系の分母係数ベクトル (s^2 + 2ζωs + ω^2)
	
	// 制御用変数宣言
	static double u = 0, y1 = 0, y2 = 0, y3 = 0;// 伝達関数の入力信号と出力信号
	static TransferFunction<2,3> G1(N, D, Ts);	// 2/3次の伝達関数1の生成
	static TransferFunction<3,3> G2(P, Q, Ts);	// 3/3次の伝達関数2の生成(直達項あり)
	static TransferFunction<0,2> G3(B, A, Ts);	// 0/2次の伝達関数3の生成(2次標準形)
