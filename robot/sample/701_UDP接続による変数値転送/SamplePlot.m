...
figure(1);
	...
	Graph1.StairsPlot(t, x5, 'Red',   'Normal');	% 線の色は Black/Gray/Red/Green/Blue の5種のみ
	%Graph1.ManualGrid(-2, 0.5, 2);					% Y軸範囲の設定(最小値, グリッド間隔, 最大値) ←コメントアウトで自動モード
	Graph1.Label({'(a)','Var. A','[-]'});			% 縦軸ラベルの設定
	Graph1.Legend({'x_1','x_2'}, 'NorthEast', 'Vertical');	% 凡例の設定
	% プロット平面2段目
	Graph1.SelectPlane(2);							% プロット段選択
	Graph1.StairsPlot(t, x3, 'Black', 'Thin');		% 線形プロット
	Graph1.StairsPlot(t, x7, 'Red',   'Normal');		% 線の色は Black/Gray/Red/Green/Blue の5種のみ
	%Graph1.ManualGrid(-2, 0.5, 2);					% Y軸範囲の設定(最小値, グリッド間隔, 最大値) ←コメントアウトで自動モード
	Graph1.Label({'(b)','Var. B','[-]'});			% 縦軸ラベルの設定
	Graph1.Legend({'x_5','x_6'}, 'NorthEast', 'Vertical');	% 凡例の設定
	% 画像生成
