...
% CSVファイルから読み込み
FileName = '../044_時間微分器/DATA.csv';	% CSVファイル名設定
...
figure(1);
	...
	Graph1.Legend({'u'}, 'NorthEast', 'Vertical');	% 凡例の設定
	% プロット平面2段目
	Graph1.SelectPlane(2);							% プロット段選択
	Graph1.LinePlot(t, x2, 'Red',   'Normal');	% 線の色は Black/Gray/Red/Green/Blue の5種のみ
	Graph1.LinePlot(t, x3, 'Green', 'Normal');	% 線の太さは Thin/Normal/Heavy の3種のみ
	Graph1.LinePlot(t, x4, 'Blue',  'Heavy');		% 重ねられるプロットの数は無制限
	...
	Graph1.Legend({'Y1(1,1)','Y1(2,1)','Y1(3,1)','Y1(1,2)','Y1(2,2)','Y1(3,2)'}, 'NorthEast', 'Vertical');	% 凡例の設定
	% 画像生成
