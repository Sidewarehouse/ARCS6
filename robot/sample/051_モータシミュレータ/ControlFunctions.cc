...
bool ControlFunctions::ControlFunction1(const double t, const double Tact, const double Tcmp){
	...
	constexpr double Kt = 0.5;		// [Nm/A] トルク定数
	constexpr double Jm = 1.00e-3;	// [kgm^2] モータ側慣性
	constexpr double Dm = 1.00e-3;	// [Nm s/rad] モータ側粘性
	
	// 制御用変数宣言
	static double iq1, wm1, thm1;	// 電流，速度，位置
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// ここに制御アルゴリズムを記述する
		std::tie(wm1, thm1) = Plant.GetSpeedAndPosition();
		iq1 = 0.3*StepWave(1, t);
		Plant.SetCurrentAndLoadTorque(iq1, 0);
		
		Interface.SetCurrent(iqref);	// [A] 電流指令ベクトルの出力
		Screen.SetVarIndicator(iq1, wm1, thm1, 0, 0, 0, 0, 0, 0, 0);	// 任意変数インジケータ(変数0, ..., 変数9)
		Graph.SetTime(Tact, t);									// [s] グラフ描画用の周期と時刻のセット
		Graph.SetVars(0, iq1, 0, 0, 0, 0, 0, 0, 0);	// グラフプロット0 (グラフ番号, 変数0, ..., 変数7)
		Graph.SetVars(1, wm1, 0, 0, 0, 0, 0, 0, 0);	// グラフプロット1 (グラフ番号, 変数0, ..., 変数7)
		Graph.SetVars(2, thm1, 0, 0, 0, 0, 0, 0, 0);	// グラフプロット2 (グラフ番号, 変数0, ..., 変数7)
		Graph.SetVars(3, 0, 0, 0, 0, 0, 0, 0, 0);	// グラフプロット3 (グラフ番号, 変数0, ..., 変数7)
		UsrGraph.SetVars(0, 0);						// ユーザカスタムプロット（例）
		Memory.SetData(Tact, t, iq1, wm1, thm1, 0, 0, 0, 0, 0, 0);		// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
