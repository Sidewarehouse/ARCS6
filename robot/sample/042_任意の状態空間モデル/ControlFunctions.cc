...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	constexpr Matrix<2,2> A = {
		 0,  1,
		-5, -2
	};
	constexpr Matrix<1,2> b = {
		0,
		3,
	};
	constexpr Matrix<2,1> C = {
		1, 0
	};
	constexpr Matrix<1,1> D = {
		1
	};
	
	// 制御用変数宣言
	static double u = 0, y1 = 0, y2 = 0;					// 状態空間モデルの入力信号と出力信号
	static StateSpaceSystem<2,1,1> System1(A, b, C, Ts);	// 状態空間モデル1
	static StateSpaceSystem<2,1,1> System2(A, b, C, D, Ts);	// 状態空間モデル2
	...
	if(CmdFlag == CTRL_LOOP){
		...
		y1 = System1.GetResponse(u);		// 状態空間モデル1の応答を計算
		y2 = System2.GetResponse(u);		// 状態空間モデル2の応答を計算
